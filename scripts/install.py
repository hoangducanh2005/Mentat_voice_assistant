...
def main() -> None:
    ...
    subprocess.run(["uv", "venv", "--python", "3.12.8", "--clear"])

    # Determine if CUDA is available and set correct paths for Windows/Unix
    is_windows = platform.system() == "Windows"
    venv_bin_dir = ".venv/Scripts" if is_windows else ".venv/bin"
    path_separator = ";" if is_windows else ":"
    abs_bin_dir = os.path.abspath(venv_bin_dir)

    # Add virtual environment to PATH
    os.environ["PATH"] = f"{abs_bin_dir}{path_separator}{os.environ['PATH']}"
    ...
    env = os.environ.copy()
    env["PATH"] = f"{abs_bin_dir}{path_separator}{env['PATH']}"
    env["VIRTUAL_ENV"] = os.path.abspath(".venv")
    
    # Run uv pip install
    subprocess.run(["uv", "pip", "install", "-e", f".{extras}"], env=env, shell=is_windows)

    # Download and verify model files
    subprocess.run(["uv", "run", "glados", "download"], env=env, shell=is_windows)
